...
Parkinson's Disease (PD) affects approximately 90% of patients through voice and speech disorders, collectively termed dysphonia. These manifestations often appear in early disease stages, sometimes predating traditional motor symptoms by several years. @little2009suitability were among the first to establish that vocal impairments in PD patients are detectable through acoustic analysis, noting that phonation is particularly affected due to the reduced control of laryngeal muscles. @tsanas2010accurate further demonstrated that voice deterioration correlates with disease progression, making vocal biomarkers valuable for both early detection and longitudinal monitoring.

The primary vocal characteristics affected by PD include reduced loudness (hypophonia), breathiness, roughness, monotonicity (reduced pitch variation), and imprecise articulation. These symptoms result from the pathophysiological changes in PD, including rigidity, bradykinesia, and tremor affecting the speech apparatus. @orozco2018neurospeech emphasized that these vocal impairments manifest consistently across languages and cultural backgrounds, confirming voice analysis as a robust, non-invasive, and cost-effective biomarker for PD screening.
...
Traditional acoustic measures include jitter (cycle-to-cycle variations in fundamental frequency), shimmer (amplitude variations), and harmonics-to-noise ratio (HNR). @sakar2013collection demonstrated that these time-domain features alone can achieve moderate classification accuracy but are most effective when combined with other feature types.

Nonlinear dynamics measures, introduced by @little2009suitability, have proven particularly valuable. These include recurrence period density entropy (RPDE), detrended fluctuation analysis (DFA), and correlation dimension, which capture the subtle nonlinear patterns in vocal fold vibrations disrupted by PD. @tsanas2011nonlinear expanded this approach with additional nonlinear measures, showing they outperform traditional acoustic parameters.
...
More recently, deep learning approaches have gained prominence. @hemmerling2016automatic implemented Convolutional Neural Networks (CNNs) and Recurrent Neural Networks (RNNs) directly on spectrograms and raw audio signals, eliminating the need for handcrafted feature extraction. Their deep neural network architecture achieved up to 95% accuracy on multiple datasets. @tracy2020smartphone further refined these approaches for smartphone-based applications, enabling real-time PD screening with minimal computational resources.
